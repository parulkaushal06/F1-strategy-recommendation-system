// Real Spa-Francorchamps outline (2007–2026 layout), traced by
// julesr0y/f1-circuits-svg (CC BY 4.0) — see /assets/circuits-svg/ATTRIBUTION.md
// at the repo root for the license and source.
export const SPA_PATH_D =
...
// S1/S2 and S2/S3 boundary positions, in this SVG's coordinate space.
// Derivation: FastF1's real position telemetry for Hamilton's fastest lap
// (2023 Belgian GP) was cross-referenced against his official Sector1/2SessionTime
// to find where his timers actually flipped on track, then registered onto this
// SVG's own outline via a corner-based similarity (Procrustes) fit against
// FastF1's 19 officially-numbered corners. Residual ~35px (~150m on the real
// track), concentrated in tightly-clustered corner sequences (Eau Rouge/Raidillon,
// Malmedy/Rivage) where curvature-peak corner detection can't cleanly separate
// closely-spaced apexes. Real and geometrically grounded, not survey-grade precise.
// This alignment is Spa-specific — extending it to another circuit means repeating
// the same fetch (FastF1 position_data + mvapi circuit info) and fit.
export const SPA_SECTOR_POINTS: [number, number][] = [
  [373.9, 335.2],
  [119.7, 440.2],
];

export const SPA_QUICK_FACTS = {
  circuitLengthKm: 7.004,
  raceDistanceKm: 308.2,
  laps: 44,
  corners: 19,
  lapRecord: "1:46.286",
  ...
  firstGrandPrix: 1950,
  drsZones: [
    { name: "Kemmel straight", detection: "Eau Rouge exit" },
    { name: "Pit straight", detection: "Turn 19 exit" },
  ],
};
